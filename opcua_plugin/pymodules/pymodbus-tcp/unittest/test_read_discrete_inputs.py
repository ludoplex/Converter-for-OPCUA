...
class TestPluginMain(unittest.TestCase):
    ...
    def test_plugin_main_read_discrete_inputs_T(self):
        uri = "127.0.0.1:5020"
        res = self.t.write_coil(uri, 20, "10")
        self.assertEqual(res['data'], 'OK')
        print("result:", res['data'])

    def test_plugin_main_read_discrete_inputs_F1(self):
        uri = "0.0.0.1:5020"
        res = self.t.write_coil(uri, 20, "10")
        self.assertEqual(res['data'], 'device not found')
        print("result:", res['data'])

    def test_plugin_main_read_discrete_inputs_F2(self):
        uri = "127.0.0.1:5020"
        res = self.t.write_coil(uri, "20", "10")
